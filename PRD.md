...
**A portfolio demo inspired by Zip's engineering blog**
...
2. **"Vibes Don't Ship"** — a rigorous eval methodology: deterministic checks
   ...
   execution trace where there isn't, and a closed loop where every production
   failure becomes a permanent new golden test case.

This project reimplements both ideas at small scale as a working demo: recipe-driven
agents, Claude tool-use, retrieval, persistence, and an eval/correction loop.
Seed content (contracts, vendor records) is demo sample data.
...
- Produce something that survives follow-up technical questions in an interview
  ("what happens if the model calls the wrong tool?" should have a concrete answer)
- Use Zip's stated stack (Python, TypeScript, React, GraphQL)
- Be finishable and demo-able solo within roughly one focused week

## 3. Non-Goals (explicitly out of scope)

- Production-grade auth, multi-tenancy, or customer production data
- A polished/branded UI — functional and clean is enough
- Full parity with Zip's production system (50+ agents, enterprise governance, SSO,
  audit trails) — this is intentionally a scaled-down proof of architecture
- Production vendor/contract corpora — demo/sample documents only
...
2. Show a full trace of the model choosing which tool to call and why
3. Run the eval suite and get non-deterministic scores from an LLM judge
4. Trigger the correction flow and show a new row landing in the golden set
5. Walk through the architecture clearly (recipes → engine → tools → eval loop)
...
| LLM | **Anthropic Claude** (Sonnet) via tool-use / Bedrock path | Zip is an Anthropic customer; native tool-calling (not manual if/else) is the orchestration model |
...
| Relational data | **SQLite** via **SQLAlchemy** | Typed schema and queries, zero setup cost; upgrade path to Postgres if needed |
...
```
   ...
   Request  ───────▶ │      GraphQL API         │
                     ...
                     │     [Claude tool-use]       │──▶ Tool: api_data (SQLite via SQLAlchemy)
...
-- Domain sample data (vendors / contracts)
...
CREATE TABLE contracts (
  ...
  file_path TEXT,         -- points to doc on disk, chunked+embedded in retrieval store
  effective_date DATE,
  renewal_date DATE,
  terms_summary TEXT
);
...
### FR2 — Tool-calling
Orchestration must use Claude's native tool-use API. The model receives
tool schemas and decides which to call, with what arguments, and
when to stop (capped at 3 iterations to bound cost/time).

### FR3 — Retrieval
`document_retrieval` must perform embedding + semantic search over
chunked documents stored in a self-hosted, on-prem Moorcheh instance
...
### FR4 — Structured data access
`api_data` must run SQL queries against the SQLite schema above via
...
run must pick it up automatically via the DB query — no manual list editing.

### FR8 — Frontend
- Request submission form
- Trace view (step-by-step tool calls + reasoning)
- Simple eval dashboard: latest scores per recipe, trend over time (from
...
- **Clarity:** README should describe the architecture and demo path clearly
  (recipes, engine, tools, eval loop)

## 11. Milestones

| Phase | Deliverable | Est. time |
|---|---|---|
| 1 | SQLite schema + seed data (demo vendors/contracts) + doc corpus sourced | 0.5 day |
| 2 | Retrieval ingestion pipeline (chunk + embed + store) working standalone | 0.5–1 day |
| 3 | Agent engine: preprocessing + tool-use orchestration + synthesis, one recipe, tested via script (no API yet) | 1–1.5 days |
...
frontend polish, demo via API/CLI) still deliver a credible architecture story.
...
> script for demo vendors/contracts, a `RetrievalBackend` interface with
...
> orchestration — tool-calling through the RetrievalBackend interface,
